<template>
  <div>
    <section class="section">
      <case :case_="case_" :caseSpec="caseSpec" />
...
export default Vue.extend({
  ...
  data() {
    return {
      ...
      defaultProviderContact: NullProviderContact() as IProviderContact,
    };
  },
  ...
  computed: {
    caseSpec(): CaseSpec {
      return {
        applicant: { editable: true, disabled: false },
        hostCountry: {
          editable: true,
          disabled: applicantIsNull(this.case_.applicant),
        },
        dateRange: {
          editable: true,
          disabled: countryIsNull(this.case_.process.route.host_country),
        },
        route: {
          editable: true,
          disabled: !(
        ...
        provider: {
          editable: true,
          disabled: routeIsNull(this.case_.process.route),
        },
        steps: { provider: { editable: true, disabled: false } },
      };
    },
  ...
  methods: {
    handleChangeApplicant(applicant: IApplicant) {
      if (!applicant) {
        // FIXME: why
    ...
    async handleChangeHostCountry(country: ICountry) {
      if (!country) {
        // FIXME: why
    ...
    handleChangeProcess(process: IProcess): void {
