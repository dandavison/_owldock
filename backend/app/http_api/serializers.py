...
class CaseSerializer(ModelSerializer):
    ...
    def get_cases_for_client_contact(
        ...
        for c in cases:
            c.process = uuid2process[c.process_uuid]
            for s in c.casestep_set.all():
                s.process_step = uuid2process_step[s.process_step_uuid]
                s._prefetched_stored_files = sorted(
                    case_step_uuid2stored_files[s.uuid], key=attrgetter("name")
                )
                if s.active_contract:
                    s.active_contract.provider_contact = uuid2provider_contact[
                        s.active_contract.provider_contact_uuid
                    ]
            a = c.applicant
            a.home_country = uuid2country[a.home_country_uuid]
            a._prefetched_nationalities = sorted(
                (
                    uuid2country[uuid]
